...
var generators map[sqlType]generator

func init() {
	generators = make(map[sqlType]generator)
...
func (c *clause) Set(name sqlType, vars ...any) {
	if c.sql == nil {
		c.sql = make(map[sqlType]string)
		c.sqlVars = make(map[sqlType][]any)
	}
...
func (c *clause) Build(orders ...sqlType) (string, []any) {
	var sqls []string
	var vars []any
